...
//		uint8_t myframedata[avctx->width * avctx->height * 3];
		...
		r = got_video_frame( encoderRescaledFrame->data[0], encoderRescaledFrame->linesize[0],
...
/*    ret = av_find_best_stream(fmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, &dec, 0);
	...
	printf( "Stream index: %d (%p %p) (ctx %p)\n", video_stream_index, dec_ctx, dec, dec_ctx );
...
//	memset( &avpkt, 0, sizeof( avpkt ) );  Nope.
	int quit = 0;
	while( !quit && av_read_frame(fmt_ctx, &avpkt) >= 0 )
	{
		if (avpkt.stream_index == video_stream_index)
		{
		    while (!quit && avpkt.size > 0)
			{
		        if (decode_write_frame( dec_ctx, frame, &frame_count, &avpkt, 0, encoderRescaledFrame))
		            quit = 1;
			}
		}
		else
